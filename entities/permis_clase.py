...
class PermisClasse(SQLModel, table=True):
    __tablename__ = "PERMISOS_CLASSES"

    usuari_id: int = Field(foreign_key="USUARIS.id", primary_key=True)
    classe_id: int = Field(foreign_key="CLASSES.id", primary_key=True)

    usuari: "Usuari" = Relationship(back_populates="permisos_classes")
    classe: "Classe" = Relationship(back_populates="permisos")
